  ...
  axios
    ...
    //   console.log(data);
      ...
      router
        ...
        .then(() => clear(waitTimeOut, stopQueryTimeOut, isSubmittingQueryForm))
        .catch(
          // 假如跳转错误，提示退出
          (error) => {
            clear(waitTimeOut, stopQueryTimeOut, isSubmittingQueryForm);
            throw Error(
              `抱歉，后端系统返回了结果，但是查询结果处理失败，错误代码是${String(
                error
              ).substr(0, 100)}。请重新提交查询再试。`
            );
          }
        );
    })
