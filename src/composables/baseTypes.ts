 ...
 * 地图的类型。
 */
export type MapType = 'street-map' | 'satellite' | 'traffic-info' | 'street-map-info' | 'satellite-info';
...
export class optionInfo {
    ...
    constructor(img: string, info: boolean, outputSize: number, outputType: string, canScale: boolean, autoCrop: boolean, autoCropWidth: number, autoCropHeight: number,  fixedBox: boolean, fixed: boolean, fixedNumber: number[], full: boolean, canMove: boolean, canMoveBox: boolean, original: boolean,
       centerBox: boolean, high: boolean, infoTrue: boolean, maxImgSize: number, enlarge: number, mode: string) {
          this.img = img;
          this.info = info;
